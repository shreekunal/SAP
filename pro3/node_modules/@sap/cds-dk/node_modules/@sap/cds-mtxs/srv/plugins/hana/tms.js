...
class TMSClient extends AbstractContainerManagerClient {
  ...
  getAll = async (tenants, options) => {
    ...
    for (const tenant of tenants) {
      ...
      if (credentials) {
        results.push({ tenant_id: tenant, credentials })
      }
    }
    return results
  }
  ...
  create = async (btpTenantName, parameters) => {
    ...
    if (!container) {
      const { location: containerLocation, container: newContainer } = await this.createContainer(hanatenant.id, btpTenantName, eTag)
  ...
  remove = async (tenant, parameters) => {
    const removed = {}
    const { cleanup_hana_tenants: cleanupHanaTenants, immediate_deletion: immediateDeletion } = parameters?.create || {}
    try {
      ...
      removed.credentials = deletions.map((_, idx) => credentials[idx].id)
      ...
      try {
        ...
        if (error.status === 404) {
          ...
          removed.container = container.id
        } else {
  ...
  _createOptions = (btpTenantName, params) => {
    ...
    if (!hanaTenantId) {
      ...
      hanaTenantPrefix = undefined // remove to indicate that hanaTenantId was predefined
    }
